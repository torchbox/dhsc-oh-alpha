...
COPY ./alpha/static_src/ ./alpha/static_src/
...
RUN useradd alpha --create-home && mkdir /app $VIRTUAL_ENV && chown -R alpha /app $VIRTUAL_ENV
...
ENV PATH=${POETRY_HOME}/bin:$VIRTUAL_ENV/bin:$PATH \
    ...
    PYTHONUNBUFFERED=1 \
    DJANGO_SETTINGS_MODULE=alpha.settings.production \
    PORT=8000 \
    WEB_CONCURRENCY=3 \
...
COPY --chown=alpha pyproject.toml poetry.lock ./
RUN pip install --upgrade pip && poetry install ${POETRY_INSTALL_ARGS} --no-root --extras gunicorn

COPY --chown=alpha --from=frontend ./alpha/static_compiled ./alpha/static_compiled

# Copy application code.
COPY --chown=alpha . .

# Run poetry install again to install our project (so the the alpha package is always importable)
...
# Load shortcuts
COPY ./docker/bashrc.sh /home/alpha/.bashrc

# Run the WSGI server. It reads GUNICORN_CMD_ARGS, PORT and WEB_CONCURRENCY
# environment variable hence we don't specify a lot options below.
CMD gunicorn alpha.wsgi:application
...
COPY --chown=alpha --from=frontend ./node_modules ./node_modules
