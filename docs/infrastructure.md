...
1. Take a backup of staging
   ```bash
   $ heroku pg:backups:capture -a alpha-staging
   ```
...
1. List production database backups:
   ```bash
   $ heroku pg:backups -a alpha-production
   ```
1. Get a signed S3 URL of your chosen backup:
   ```bash
   $ heroku pg:backups:url {backup-name} -a alpha-production
   ```
1. Upload to staging:
   ```bash
   $ heroku pg:backups:restore {backup-url} DATABASE_URL -a alpha-staging
   ```
...
1. Check CI is working e.g. https://git.torchbox.com/[client_name]/alphapipelines
